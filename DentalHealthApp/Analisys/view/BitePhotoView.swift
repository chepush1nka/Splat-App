...
struct BitePhotoView: View {
    ...
    private func updateData() {
        ...
        case 3:
            ...
            viewModel.performAnalysis(type: "bite", images: approvedImages)
        default:
            self.header = ""
            self.description = ""
        }
    ...
    var body: some View {
            ...
            VStack {
                ...
                if approvedImages.count < 3 {
                    ...
                    .actionSheet(isPresented: $showSheet) {
                        ActionSheet(title: Text("Загрузите фото"),
                        message: Text("Выберите фото из галлереи или сфотографируйтесь"), buttons: [
                            .default(Text("Галлерея")) {
        ...
        .fullScreenCover(isPresented: $viewModel.showResultScreen) {
            ResultsView(callback:  {
                self.presentationMode.wrappedValue.dismiss()
                self.callback()
            }, result: $viewModel.analysisResult.wrappedValue)
        }
        .fullScreenCover(isPresented: $viewModel.showErrorScreen) {
            ErrorView(callback:  {
                self.presentationMode.wrappedValue.dismiss()
                self.callback()
            })
